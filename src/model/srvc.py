...
class SRVC(nn.Module):
    def __init__(self, args):
        ...
        # m_head = [nn.Conv2d(3,256,kernel_size=3,stride=1,padding=0,bias=True),
        #           nn.ReLU(True)]
        ...
        # define adaConv
        self.kernel = nn.Conv2d(self.f,27*self.F,kernel_size=3,stride=1,padding=0,bias=True)
        self.bias = nn.Conv2d(self.f,self.F,kernel_size=3,stride=1,padding=0,bias=True)
        ...
        m_body = [nn.Conv2d(self.F,128,kernel_size=5,stride=1,padding=2,bias=True),
                  nn.ReLU(True),
                  nn.Conv2d(128,32,kernel_size=3,stride=1,padding=1,bias=True),
                  nn.ReLU(True),
                  nn.Conv2d(32,3*scale*scale,kernel_size=3,stride=1,padding=1,bias=True)]
        # m_body = [common.MyResBlock(common.default_conv, self.F, self.n_feats, 5, bias=True, bn=False),
